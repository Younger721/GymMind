...
class ElasticsearchKeywordIndexTest {
 ...
 @Test void createsMissingIndexBeforeFirstDocument(){var builder=RestClient.builder();var server=MockRestServiceServer.bindTo(builder).build();var index=new ElasticsearchKeywordIndex(builder);server.expect(requestTo("http://localhost:9200/gymmind-knowledge")).andRespond(withStatus(HttpStatus.NOT_FOUND));server.expect(requestTo("http://localhost:9200/gymmind-knowledge")).andRespond(withSuccess());server.expect(requestTo("http://localhost:9200/gymmind-knowledge/_doc/c1")).andRespond(withSuccess());index.upsert(new IndexedChunk("c1",7L,"text",new float[1024]));server.verify();}
}
