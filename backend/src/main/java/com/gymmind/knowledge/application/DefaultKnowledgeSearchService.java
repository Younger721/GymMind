...
@Service public class DefaultKnowledgeSearchService implements KnowledgeSearchService {
 ...
 public List<ContextSegment> search(CurrentActor a,String q){
  ...
  List<ContextSegment> fused=RrfFusion.merge(kr,vr,20).stream().map(x->all.get(x.id())).filter(Objects::nonNull).map(c->new ContextSegment(c.id(),c.id(),c.text())).toList();
  if(rerank!=null&&!fused.isEmpty())try{Map<String,ContextSegment> byId=new HashMap<>();fused.forEach(c->byId.put(c.id(),c));List<RankedChunk> ranked=rerank.rerank(q,fused);if(ranked!=null&&!ranked.isEmpty())fused=ranked.stream().map(x->byId.get(x.id())).filter(Objects::nonNull).toList();}catch(RuntimeException ignored){}
